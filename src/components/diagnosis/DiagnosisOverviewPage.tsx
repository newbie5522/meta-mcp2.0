...
export function DiagnosisOverviewPage() {
  ...
  const failedDetectors = reportMeta?.failedDetectors || [];
       ...
       <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center space-y-4 max-w-3xl mx-auto">
         ...
         <div className="space-y-2">
           ...
           <p className="text-xs text-slate-400 max-w-md mx-auto">
             {reportMessage || "当前日期范围内没有触发生产建议、数据健康提醒或规则过滤项。系统不会注入 mock/demo 数据。"}
           </p>
           {failedDetectors.length > 0 && (
             <p className="text-[11px] text-orange-500 max-w-md mx-auto">
               其中 {failedDetectors.length} 个诊断模块已降级跳过，请先查看上方橙色提示。
             </p>
           )}
         </div>
         <button
