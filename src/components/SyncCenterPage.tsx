...
export function SyncCenterPage() {
  ...
  const translateTaskType = (t: string) => {
    const dict: Record<string, string> = {
      sync_store_profile: "店铺基本资料 (Store Profile)",
      sync_store_orders: "订单财务流水 (Store Orders)",
      sync_meta_accounts: "广告主体检测 (Meta Accounts)",
      sync_meta_activity: "90天活跃检测 (Activity Filter)",
      sync_meta_structure: "广告结构透视 (Meta Structures)",
      sync_meta_insights: "广告消耗指标 (Meta Insights)",
      rebuild_store_summary: "重构店铺日汇总 (Store Summary)",
      rebuild_meta_summary: "重构Meta成效汇总 (Meta Summary)",
      rebuild_roas_summary: "重组真数ROAS归类 (Real ROAS Mapping)",
      rebuild_dashboard_summary: "总计Dashboard成效 (Global Dashboard)" ,
      run_ai_rule_monitor: "AI规则扫描与风控体检 (AI Rule Monitor)"
    };
    return dict[t] || t;
  };
              ...
              <div className="space-y-2">
                <div className="flex gap-1.5">
                  <select
                    ...
                    className="flex-1 text-xs bg-white border border-slate-200 rounded-lg px-2 text-slate-700 focus:outline-none"
                  >
                    {stores.map((s) => (
                ...
                <button
                  onClick={() => handleTriggerTask("rebuild_roas_summary")}
                  disabled={!!isTriggering}
                  className="w-full flex items-center justify-between text-left p-2 bg-white hover:bg-slate-100 border border-slate-200/80 rounded-lg text-xs font-semibold text-slate-700 transition"
                >
                  <span>4. 结合映射重算 Real ROAS</span>
                  <Play className="w-3.5 h-3.5 text-indigo-500 fill-indigo-500" />
                </button>

                <button
                  onClick={() => handleTriggerTask("run_ai_rule_monitor")}
                  disabled={!!isTriggering}
                  className="w-full flex items-center justify-between text-left p-2 bg-white hover:bg-slate-100 border border-slate-200/80 rounded-lg text-xs font-semibold text-slate-700 transition"
                >
                  <span>5. 触发 AI 风控分析与体检扫描</span>
